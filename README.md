...
2. The `Humblee` has 2h to pay the `Lottery Pool` which is set to `2% fee` of `Total Followers` (can be modified / ajusted by the `DAO`). 

3. If `Humblee` paies the ransom to the `Lottery Pool`, the smartcontract then triggers `Humiliation Mechanism` where the `Humblee` has to appology on Twitter. He can then signal another `Humblee` to send the potato to. The final decision is given by the `DAO`. 

4. If `Humblee` doesn't want to pay the ransom, the smartcontract triggers the `Suffering Period`, it's `7 days` where people can join the `Lottery Pool` to unfollow the `Humblee`. 
